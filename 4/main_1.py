...
for guard_id in guard_ids:
    guard = guards[guard_id]
    total_sleep.append(guard.sleep_counter.sum())

top_guard = guard_ids[np.argmax(total_sleep)]
print(int(top_guard))
print(guards[top_guard].sleep_counter)

print(int(top_guard) * np.argmax(guards[top_guard].sleep_counter))
